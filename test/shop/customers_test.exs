defmodule Shop.CustomersTest do
  ...
  alias Shop.Customers.{Customer, CustomerToken}
  ...
  describe "get_customer_by_email_and_password/2" do
    ...
    test "returns the customer if the email and password are valid" do
      ...
      assert %Customer{id: ^id} =
               Customers.get_customer_by_email_and_password(customer.email, valid_customer_password())
    end
  end
    ...
    test "sends token through notification", %{customer: customer} do
      token =
        extract_customer_token(fn url ->
          Customers.deliver_customer_update_email_instructions(customer, "current@example.com", url)
        end)
  ...
  describe "update_customer_email/2" do
    setup do
      ...
      token =
        extract_customer_token(fn url ->
          Customers.deliver_customer_update_email_instructions(%{customer | email: email}, customer.email, url)
        end)
  ...
  describe "get_customer_by_session_token/1" do
    ...
    test "returns customer by token", %{customer: customer, token: token} do
      assert {session_customer, token_inserted_at} = Customers.get_customer_by_session_token(token)
